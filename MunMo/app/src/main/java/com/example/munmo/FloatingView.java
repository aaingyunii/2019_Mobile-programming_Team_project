...
class FloatingView extends FrameLayout implements ViewTreeObserver.OnPreDrawListener {
    ...
    public boolean onPreDraw() {
        ...
        // Enter default values if initial values are set for X coordinates (no margin is considered)
        if (mInitX == DEFAULT_X) {
            mInitX = 0;
        }
        // Enter default values if initial values are set in Y coordinates
        if (mInitY == DEFAULT_Y) {
    ...
    private void updateNavigationBarOffset(boolean isHideNavigationBar, boolean isPortrait, Rect windowRect) {
        ...
        if (!isHideNavigationBar) {
            ...
            // 1. The navigation bar embedded in the device (mBaseNavigationBarHeight == 0) does not vary
            // in height depending on the system state.
            // 2. The navigation bar (!hasSoftNavigationBar) built into the device is contradictory
            // because it intentionally sets the Base to 0.
    ...
    private void refreshLimitRect() {
        ...
        mMoveLimitRect.set(-width, -height * 2, newScreenWidth + width + mNavigationBarHorizontalOffset,
                newScreenHeight + height + mNavigationBarVerticalOffset);

        mPositionLimitRect.set(-mOverMargin, 0, newScreenWidth - width + mOverMargin + mNavigationBarHorizontalOffset,
    ...
    public boolean dispatchTouchEvent(@NonNull MotionEvent event) {
        // Do nothing unless View is displayed
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            // To prevent MOVE or the like from being processed
            // when the flag of mIsDragable or getVisibility() is changed after depression.
        ...
        // migration
            ...
            // Do not do so if the press-down process has not been performed
            if (mTouchDownTime != event.getDownTime()) {
                return true;
            }
            // Not in the move acceptance state and both X and Y axes are smaller than the threshold.
        ...
        // push up, cancel
            ...
            // Temporarily hold hold on to a decision
            final boolean tmpIsLongPressed = mIsLongPressed;
            // release of long push
            mIsLongPressed = false;
            mLongPressHandler.removeMessages(LongPressHandler.LONG_PRESSED);
    ...
    // Moves to the specified coordinates.<br/>
    // If you exceed the coordinates at the edge of the screen,
    // you will automatically be directed to the edge of the screen.
    private void moveTo(int currentX, int currentY, int goalPositionX, int goalPositionY, boolean withAnimation) {

        // Adjusted to stay away from the edge of the screen.
        ...
        if (withAnimation) {
            ...
            // Refresh only when position changes
...
//         * Start Physics-based animation
    ...
    private void setScale(float newScale) {
        // INFO:Child is modified to address scenarios where scaling will not change
    ...
    static class FloatingAnimationHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            // Initialize when State Change or Animation is initiated
            if (mIsChangeState || animationType == TYPE_FIRST) {
                // use animation time only when changing states
            ...
            // animation in the case of overlapping
                ...
                //movement from the current location
                params.x = (int) (mStartX + (targetPositionX - mStartX) * basePosition);
                params.y = (int) (mStartY + (targetPositionY - mStartY) * basePosition);
                floatingView.updateViewLayout();
                sendMessageAtTime(newMessage(animationCode, TYPE_UPDATE), SystemClock.uptimeMillis() + ANIMATION_REFRESH_TIME_MILLIS);
        ...
        /**
         * It calculates the location of the animation time.。
        */
        ...
        void setState(@AnimationState int newState) {
            // Change state flag only if the state is different
            if (mState != newState) {
                mIsChangeState = true;
            }
            mState = newState;
    ...
    /**
     * It's a handler that controls the long-press process.。<br/>
     * We implement all touch processing in dispatchTouchEvent, so we also implement a proprietary push.
     * */
